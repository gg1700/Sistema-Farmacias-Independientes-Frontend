...
import { RegisterTableProps } from "../../types/RegisterTable.types";
import { gridMap } from "../../constants/table";
import ActionButton from "../ui/ActionButton";  

function RegisterTable({ subtitle, header, registers, pagination, tableStyles, buttonStyles }: RegisterTableProps) {
  const [page, setPage] = useState(0);
  
  const columns = header.length;
  const columnStyle = gridMap.get(columns);
  ...
  return (
    <section className="flex flex-col justify-center items-center p-3">
      <h2 className="text-black text-center font-medium text-2xl p-5">{subtitle}</h2>
      <div className={`bg-modals grid ${tableStyles} rounded-2xl shadow-sm`} lang="es">  
        <ul className={`grid ${columnStyle} gap-2 text-center items-center p-4 border-b-2 border-[#ADA87F]`}>
          {
            header.map((h: any) => (
...

        {
          currentPage.map((rgtr: any) => (
            <ul key={rgtr.id} className={`grid ${columnStyle} gap-2 text-center items-center p-4 h-16`}>
              {
                Object.keys(rgtr)
...

        {
          ((remainingRegisters !== 0) && (page === totalPages)) ?
            Array.from({ length: remainingRegisters }, (_, i) => (
              <ul key={`void-row-${subtitle}-${i}`} className={`grid ${columnStyle} gap-2 text-center items-center p-4 h-16`}>
                {
      ...
      <div className={`flex items-center justify-center ${buttonStyles} px-3 py-5`}>
